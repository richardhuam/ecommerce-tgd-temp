...
import { useGetBestSellers } from '@/shared/queries/product/product.query';

type ProductBestSellersSectionProps = {};

export default function ProductBestSellersSection({}: ProductBestSellersSectionProps) {
  const router = useRouter();
  const [page, setPage] = useState<number>(1);
  const { data: bestSellers, isLoading } = useGetBestSellers({ page });

  const bestSellersData = bestSellers?.data.result ?? [];
  ...
  return (
    <div className="container px-1 md:px-4">
      ...
      {bestSellers?.data.result.length === 0 && !isLoading ? (
        ...
        <>
          ...
          {bestSellers?.data.pagination && (
            <div className="mt-4 lg:mt-6">
              <Paginate onPageChange={handlePageChange} pagination={bestSellers.data.pagination} />
            </div>
          )}
        </>
      )}
